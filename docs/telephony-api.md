...
| `webhook_secret` | string | no | Shared secret for webhook verification. **Write-only** — accepted on create/update, never echoed back. Reads return `webhook_secret_configured` (boolean) instead, so a client can tell whether one is set without being able to read it. |
...
Returns the configuration a WebRTC client needs to register against TeleCMI's SBC.

> ### ⚠️ This endpoint RETURNS THE SIP PASSWORD IN PLAINTEXT
>
> Earlier revisions of this document stated the password is "never returned by
> this endpoint". **That was wrong**, and a security reviewer reading only the
> old text would have reached the opposite conclusion about this endpoint's
> risk. It is returned, deliberately, in `auth.value`.
>
> The reasoning is in `telephony/views.py` above the response: a WebRTC client
> registers directly against the SBC and must present SIP credentials itself.
> A TeleCMI login token is not an alternative — the SBC wants the password.
> In the common case it is a *shared tenant extension* password rather than a
> per-user one.
>
> What follows from that, and is worth stating plainly:
> * any authenticated user who can reach this endpoint can read that password;
> * because it is often shared per tenant, it is not scoped to the caller;
> * it is returned on every call — there is deliberately no server-side cache.
>
> Treat it as a credential-issuing endpoint, not a config endpoint.
...
```json
{
  ...
  "sbc_host": "sbcind.telecmi.com",
  "default_caller_id": "+918000000000",
  "auth": { "kind": "password", "value": "<SIP password in plaintext>" },
  "source": "profile"
}
```
...
| Field | Notes |
|---|---|
| `telecmi_user_id` | SIP username / extension. |
| `sbc_host` | SBC to register against, e.g. `sbcind.telecmi.com` (WSS). |
| `default_caller_id` | The resolved calling profile's number when there is one, else the tenant-wide default. |
| `auth.kind` | Currently always `password`. Present so a token-based scheme can be added without breaking clients. |
| `auth.value` | **The SIP password, plaintext.** See the warning above. |
| `source` | Where the identity was resolved from (e.g. calling profile vs tenant default). |
...
## 15b. Features previously undocumented

Everything below is implemented and routed, but had no entry in this document.
Routes ending in an action name (`/recording/`, `/assign/`, …) are DRF
`@action` routes generated by the router — they will not appear as `path()`
entries in `telephony/urls.py`.

### Calling profiles

A calling profile is a reusable SIP identity (extension + password + caller ID)
that can be assigned to users, so several agents can share one TeleCMI
extension without each holding their own credentials.

| Method | Path | Notes |
|---|---|---|
| GET / POST | `/api/telephony/calling-profiles/` | List; create is admin-only. |
| PATCH / DELETE | `/api/telephony/calling-profiles/<pk>/` | Update or remove. |
| POST | `/api/telephony/calling-profiles/<pk>/verify/` | Test the extension + password against TeleCMI before relying on it. |
| POST / DELETE | `/api/telephony/calling-profiles/<pk>/assign/` | Assign or unassign a user. |
| GET | `/api/telephony/calling-profiles/assignments/` | Current user → profile map. |

`GET /webrtc-config/` resolves through these: its `source` field says whether
the identity came from a profile or the tenant default.

### Call outcome (disposition)

| Method | Path | Notes |
|---|---|---|
| PATCH | `/api/telephony/calls/<pk>/outcome/` | Sets `call_outcome`, `call_outcome_note`, `call_outcome_set_at` on the call. |

### Analytics

| Method | Path | Notes |
|---|---|---|
| GET | `/api/telephony/analytics/` | Tenant-wide dashboard figures. |
| GET | `/api/telephony/analytics/daily/` | Per-agent, per-day statistics. |

Both are computed from our own `CallLog` rows. They are **not** reconciled
against TeleCMI's `/v2/analysis`; see `docs/TELEPHONY_ANALYTICS_PLAN.md`.

### Campaigns

| Method | Path | Notes |
|---|---|---|
| GET / POST | `/api/telephony/campaigns/` | List / create. |
| PATCH / DELETE | `/api/telephony/campaigns/<pk>/` | Update / delete. |
| POST | `/api/telephony/campaigns/<pk>/push-leads/` | Push a set of leads into the campaign. |
| POST | `/api/telephony/campaigns/<pk>/push-group/` | Push a lead group. |
| POST | `/api/telephony/campaigns/<pk>/toggle-active/` | Start / pause. |

### Call recordings

| Method | Path | Notes |
|---|---|---|
| GET | `/api/telephony/calls/<pk>/recording/` | Proxy-streams the audio through the backend, from TeleCMI or Zata depending on where it lives. The client never needs storage credentials. |
| GET | `/api/telephony/calls/<pk>/recording-access/` | Returns a presigned URL instead, for clients that would rather fetch the object directly. |

`CallLog.recording_file` holds the provider-side filename (e.g.
`demo_1111113.wav`); it is not a URL.

### Zata storage credentials

| Method | Path | Notes |
|---|---|---|
| GET / POST | `/api/telephony/storage-credentials/` | Per-tenant object-storage credentials used to archive recordings and to presign access. |

### Device tokens (push)

| Method | Path | Notes |
|---|---|---|
| POST / DELETE | `/api/telephony/device-tokens/` | Register or remove an FCM token. |

Used to wake a device on an inbound `ringing` live event, before the SBC's own
ring times out. The `app` field records which client registered the token
(`crmflutter`, `celiyocrmmobileapp`), so one user on two apps gets both woken.
...
| `telephony/models.py` | `TeleCMICredential`, `TeleCMIAgent`, `TeleCMICallingProfile`, `TeleCMIProfileAssignment`, `ZataStorageCredential`, `CallLog`, `SMSLog`, `TeleCMICampaign`, `DeviceToken`, `TeleCMIWebhookLog` |
...
| `telephony/migrations/0003` … `0012` | Calling profiles, Zata credentials, campaigns, call outcome, device tokens, and `TeleCMIWebhookLog`. Twelve migrations exist; this table lists only the notable ones. |
...
| `telephony/services/analytics_service.py` | Per-agent / per-day call statistics |
| `telephony/services/campaign_service.py` | Outbound campaign orchestration |
| `telephony/services/softphone_service.py` | Resolves the WebRTC identity returned by `/webrtc-config/` |
| `telephony/services/push_service.py` | FCM wake pushes for inbound ringing |
| `telephony/services/realtime.py` | Pusher publish for live call events |
| `telephony/services/zata_storage.py` | Recording storage/presigned access via Zata |
| `telephony/services/crypto.py` | Encrypt/decrypt stored agent passwords |
